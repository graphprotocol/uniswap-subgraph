...
export function handleTokenPurchase(event: TokenPurchase): void {
  ...
  let ethAmount = event.params.eth_sold
    .toBigDecimal()
  ...
  if (exchange.tokenDecimals == null) {
    ...
    tokenAmount = event.params.tokens_bought
      .toBigDecimal()
  ...
  exchange.combinedBalanceInEth = exchange.ethBalance
    .plus(exchange.tokenBalance.div(exchange.price))
    .truncate(18)
  ...
  exchange.totalValue = exchange.totalValue
    .plus(tokenAmount.times(exchange.price))
    .truncate(18)
  exchange.weightedAvgPrice = exchange.totalValue
    .div(exchange.tradeVolumeToken)
    .truncate(18)
  ...
  userExchangeData.tokensBought = userExchangeData.tokensBought.plus(
    tokenAmount
  )

  let originalEthValue = ethAmount.div(
    BigDecimal.fromString("1").minus(exchange.fee)
  )
  ...
  if (oneUSDInEth.equals(BigDecimal.fromString("0"))) {
    ...
    if (exchange.price.equals(BigDecimal.fromString("0"))) {
      ...
      exchange.priceUSD = BigDecimal.fromString("1")
        ...
        .div(exchange.price)
        .truncate(4)
      exchange.combinedBalanceInUSD = exchange.combinedBalanceInEth
        .div(oneUSDInEth)
        .truncate(4)
    }
  ...
  uniswap.exchangeHistoryEntityCount = uniswap.exchangeHistoryEntityCount.plus(
    BigInt.fromI32(1)
  )
  ...
  let eh = new ExchangeHistoricalData(
    uniswap.exchangeHistoryEntityCount.toString()
  )
  ...
  if (exchange.ethBalance.equals(BigDecimal.fromString("0"))) {
    ...
    exchangeDayData.marginalEthRate = exchange.tokenBalance
      .div(exchange.ethBalance)
      .truncate(8)
  }
  ...
  exchangeDayData.totalEvents = exchangeDayData.totalEvents.plus(
    BigInt.fromI32(1)
  )
...
export function handleEthPurchase(event: EthPurchase): void {
  ...
  let exchangeID = event.address.toHex()
  let exchange = Exchange.load(exchangeID)
  let ethAmount = event.params.eth_bought
    .toBigDecimal()
    .div(exponentToBigDecimal(18))
  let tokenAmount: BigDecimal
  if (exchange.tokenDecimals == null) {
    ...
    tokenAmount = event.params.tokens_sold
      .toBigDecimal()
      .div(exponentToBigDecimal(exchange.tokenDecimals))
  }
  ...
  // Fee Calculations
  let originalTokenValue = tokenAmount.div(
    BigDecimal.fromString("1").minus(exchange.fee)
  )
  ...
  if (oneUSDInEth.equals(BigDecimal.fromString("0"))) {
    ...
    exchange.weightedAvgPriceUSD = BigDecimal.fromString("1000000000000000000")
      ...
      .div(exchange.weightedAvgPrice)
      .truncate(4)
    userExchangeData.tokenFeesInUSD = BigDecimal.fromString(
      "1000000000000000000"
    )
...
export function handleAddLiquidity(event: AddLiquidity): void {
  ...
  let exchangeID = event.address.toHex()
  let exchange = Exchange.load(exchangeID)
  let ethAmount = event.params.eth_amount
    .toBigDecimal()
    .div(exponentToBigDecimal(18))
  let tokenAmount: BigDecimal
  if (exchange.tokenDecimals == null) {
    ...
    tokenAmount = event.params.token_amount
      .toBigDecimal()
      .div(exponentToBigDecimal(exchange.tokenDecimals))
  }
  exchange.ethBalance = exchange.ethBalance.plus(ethAmount)
  ...
  exchange.addLiquidityCount = exchange.addLiquidityCount.plus(
    BigInt.fromI32(1)
  )
  exchange.lastPrice = exchange.price
  ...
  userExchangeData.tokensDeposited = userExchangeData.tokensDeposited.plus(
    tokenAmount
  )
  ...
  uniswap.totalLiquidityInEth = uniswap.totalLiquidityInEth.plus(
    ethAmount.times(BigDecimal.fromString("2"))
  )
  ...
  let liquidityEventID = uniswap.totalAddLiquidity.plus(
    uniswap.totalRemoveLiquidity
  )
...
export function handleRemoveLiquidity(event: RemoveLiquidity): void {
  ...
  exchange.removeLiquidityCount = exchange.removeLiquidityCount.plus(
    BigInt.fromI32(1)
  )
  exchange.lastPrice = exchange.price
  ...
  userExchangeData.tokensWithdrawn = userExchangeData.tokensWithdrawn.plus(
    tokenAmount
  )
  ...
  uniswap.totalLiquidityUSD = uniswap.totalLiquidityInEth
    ...
    .times(exchange.priceUSD)
    .truncate(4)
  uniswap.totalRemoveLiquidity = uniswap.totalRemoveLiquidity.plus(
    BigInt.fromI32(1)
  )
  uniswap.exchangeHistoryEntityCount = uniswap.exchangeHistoryEntityCount.plus(
    BigInt.fromI32(1)
...
export function handleTransfer(event: Transfer): void {
  ...
  let uniTokens = event.params._value
    .toBigDecimal()
    .div(exponentToBigDecimal(18))
  ...
  if (
    ...
    event.params._to.toHex() == "0x0000000000000000000000000000000000000000"
  ) {
    ...
    if (exchange.totalUniToken == new BigDecimal(new BigInt(0))) {
      log.error(
        "exchange.totalUniToken is zero, ignoring transfer",
        new Array()
      )
      return
    }
